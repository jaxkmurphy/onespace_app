...
class _OddOneOutPageState extends State<OddOneOutPage> {
  ...
  OddOneOutRound? get _currentRound {
  ...
  Future<void> _selectPack(OddOneOutPack pack) async {
    ...
    try {
      ...
      if (playableRounds.isEmpty) {
        ...
        _showMessage('This pack has no playable rounds yet.');
        return;
      }
      ...
      _showMessage('Could not load that pack.');
    }
  }
  ...
  void _restart() {
    final shuffled = List<OddOneOutRound>.from(_rounds)..shuffle(_random);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Odd One Out')),
      body: Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Round ${_roundIndex + 1} of ${_rounds.length} • Score $_score',
                  style: const TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.w700,
                  ),
  ...
  Widget _buildFeedback(Color color, OddOneOutRound round) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            'The odd one out was ${round.items[round.oddIndex].label}.',
            textAlign: TextAlign.center,
